...
rdAdvancedProperties::rdAdvancedProperties()
{
	m_dlgvisible = FALSE;
	m_usersettings = TRUE;
}
...
// Initialization
BOOL
rdAdvancedProperties::Init(rdServer *server)
{
...
void
rdAdvancedProperties::Show(BOOL show, BOOL usersettings)
{
	if (show)
	{
		...
		{
			if (usersettings)
				rdlog.Print(LL_INTINFO, rdLOG("show per-user Advanced Properties\n"));
			else
				rdlog.Print(LL_INTINFO, rdLOG("show default system Advanced Properties\n"));
...
/*
BOOL rdAdvancedProperties::DoDialog()
{
...
BOOL CALLBACK
rdAdvancedProperties::DialogProc(HWND hwnd,
						  UINT uMsg,
						  WPARAM wParam,
						  LPARAM lParam )
{
	...
	rdAdvancedProperties *_this = (rdAdvancedProperties *) GetWindowLong(hwnd, GWL_USERDATA);
	...
	case WM_INITDIALOG:
		{
			...
			_this = (rdAdvancedProperties *) lParam;

			// Set the dialog box's title to indicate which Properties we're editting
			if (_this->m_usersettings) {
				SetWindowText(hwnd, "Winrd: Current User Advanced Properties");
			} else {
				SetWindowText(hwnd, "Winrd: Default Local System Advanced Properties");
			}
...
LONG
rdAdvancedProperties::LoadInt(HKEY key, LPCSTR valname, LONG defval)
{
	LONG pref;
...
char *
rdAdvancedProperties::LoadString(HKEY key, LPCSTR keyname)
{
	DWORD type = REG_SZ;
	DWORD buflen = 0;
	BYTE *buffer = 0;
...
void
rdAdvancedProperties::Load(BOOL usersettings)
{
	char username[UNLEN+1];
...
void
rdAdvancedProperties::LoadUserPrefs(HKEY appkey)
{
...
void
rdAdvancedProperties::ApplyUserPrefs()
{
...
void
rdAdvancedProperties::SaveInt(HKEY key, LPCSTR valname, LONG val)
{
	RegSetValueEx(key, valname, 0, REG_DWORD, (LPBYTE) &val, sizeof(val));
}

void
rdAdvancedProperties::Save()
{
	HKEY appkey;
	DWORD dw;
	...
	SaveInt(hkLocal, "DebugMode", rdlog.GetMode());
	SaveInt(hkLocal, "DebugLevel", rdlog.GetLevel());
...
void
rdAdvancedProperties::SaveUserPrefs(HKEY appkey)
{
