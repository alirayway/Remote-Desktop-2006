// *** JNW
// THIS VERSION OF rdDESKTOP TRIES TO USE DIRECTX TO INCREASE PERFORMANCE.
...
rdDesktopThread::run_undetached(void *arg)
{
	...
	// locking the rdServer object.  But the rdServer object is already locked
	...
	while (TRUE)
	{
		...
		if (!PeekMessage(&msg, m_desktop->Window(), NULL, NULL, PM_NOREMOVE))
		{
			...
			if (m_desktop->m_displaychanged || !rdService::InputDesktopSelected())
			{
				...
				rdlog.Print(LL_INTINFO, rdLOG("SCR: old screen format "
												"%dx%dx%d\n"),
							 oldscrinfo.framebufferWidth,
							 oldscrinfo.framebufferHeight,
							 oldscrinfo.format.bitsPerPixel);
				rdlog.Print(LL_INTINFO, rdLOG("SCR: new screen format "
												"%dx%dx%d\n"),
...
{
	// *** TIMING
	rdlog.Print(LL_INTERR, rdLOG("TIMINGS : captureticks = %d\n"), captureticks);
	rdlog.Print(LL_INTERR, rdLOG("TIMINGS : updateticks = %d\n"), updateticks);

	rdlog.Print(LL_INTINFO, rdLOG("killing screen server\n"));
...
// This routine calls the rdNTOnly class, which implements NT specific Winrd functions
...
rdDesktop::InitBitmap()
{
	...
	try {
		...
		if (DDSdesc.dwFlags & DDSD_WIDTH)
			rdlog.Print(LL_INTERR, rdLOG("surface_w = %d\n"), DDSdesc.dwWidth);
		if (DDSdesc.dwFlags & DDSD_HEIGHT)
			rdlog.Print(LL_INTERR, rdLOG("surface_h = %d\n"), DDSdesc.dwHeight);
		if (DDSdesc.dwFlags & DDSD_PITCH)
			rdlog.Print(LL_INTERR, rdLOG("surface_pitch = %d\n"), DDSdesc.lPitch);
		if (DDSdesc.dwFlags & DDSD_PIXELFORMAT)
		{
			rdlog.Print(LL_INTERR, rdLOG("surface_rgb = %s\n"),
				DDSdesc.ddpfPixelFormat.dwFlags & DDPF_RGB ? "yes" : "no");
			rdlog.Print(LL_INTERR, rdLOG("surface_rmask = %x\n"),
				DDSdesc.ddpfPixelFormat.dwRBitMask);
			rdlog.Print(LL_INTERR, rdLOG("surface_gmask = %x\n"),
				DDSdesc.ddpfPixelFormat.dwGBitMask);
			rdlog.Print(LL_INTERR, rdLOG("surface_bmask = %x\n"),
				DDSdesc.ddpfPixelFormat.dwBBitMask);
		}

		rdlog.Print(LL_INTERR, rdLOG("DirectX started OK.\n"));
	} catch (const char *str) {
		rdlog.Print(LL_INTERR, rdLOG("%s\n"), str);
		return FALSE;
	};
	...
	{
		rdlog.Print(LL_INTERR, rdLOG("DX - surface dimensions not given\n"));
		return FALSE;
	}
...
{
	rdlog.Print(LL_INTINFO, rdLOG("initialising desktop handler\n"));
...
rdDesktop::CaptureScreen(RECT &rect, BYTE *scrBuff, UINT scrBuffSize)
{
	...
	// rdlog.Print(LL_INTERR, rdLOG("surface pitch = %d\n"), surfdesc.lPitch);
