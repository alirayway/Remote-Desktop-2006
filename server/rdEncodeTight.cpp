...
// This file implements the rdEncoder-derived rdEncodeTight class.
...
const TIGHT_CONF rdEncodeTight::m_conf[10] = {
...
UINT
rdEncodeTight::RequiredBuffSize(UINT width, UINT height)
{
...
UINT
rdEncodeTight::NumCodedRects(RECT &rect)
{
...
UINT
rdEncodeTight::EncodeRect(BYTE *source, VSocket *outConn, BYTE *dest,
						   const RECT &rect)
{
...
void
rdEncodeTight::FindBestSolidArea(BYTE *source, int x, int y, int w, int h,
...
void rdEncodeTight::ExtendSolidArea(BYTE *source, int x, int y, int w, int h,
									 CARD32 colorValue,
									 int *x_ptr, int *y_ptr,
									 int *w_ptr, int *h_ptr)
{
...
bool
rdEncodeTight::CheckSolidTile(BYTE *source, int x, int y, int w, int h,
...
#define DEFINE_CHECK_SOLID_FUNCTION(bpp)									  \
																			  \
bool 																		  \
rdEncodeTight::CheckSolidTile##bpp(BYTE *source, int x, int y, int w, int h, \
									CARD32 *colorPtr, bool needSameColor)	  \
{																			  \
	CARD##bpp *fbptr;														  \
	CARD##bpp colorValue;													  \
...
UINT
rdEncodeTight::EncodeRectSimple(BYTE *source, VSocket *outConn, BYTE *dest,
								 const RECT &rect)
{
...
UINT
rdEncodeTight::EncodeSubrect(BYTE *source, VSocket *outConn, BYTE *dest,
							  int x, int y, int w, int h)
{
	...
	if (encDataSize < 0)
		return rdEncoder::EncodeRect(source, dest, r);
...
void
rdEncodeTight::SendTightHeader(int x, int y, int w, int h)
{
...
int
rdEncodeTight::SendSolidRect(BYTE *dest)
{
	int len;
...
int
rdEncodeTight::SendMonoRect(BYTE *dest, int w, int h)
{
...
int
rdEncodeTight::SendIndexedRect(BYTE *dest, int w, int h)
{
...
int
rdEncodeTight::SendFullColorRect(BYTE *dest, int w, int h)
{
...
int
rdEncodeTight::SendGradientRect(BYTE *dest, int w, int h)
{
...
int
rdEncodeTight::CompressData(BYTE *dest, int streamId, int dataLen,
							 int zlibLevel, int zlibStrategy)
{
	...
	if (!m_zsActive[streamId]) {
		...
		rdlog.Print(LL_INTINFO,
					 rdLOG("calling deflateInit2 with zlib level:%d\n"),
					 zlibLevel);
		int err = deflateInit2 (pz, zlibLevel, Z_DEFLATED, MAX_WBITS,
								MAX_MEM_LEVEL, zlibStrategy);
		if (err != Z_OK) {
			rdlog.Print(LL_INTINFO,
						 rdLOG("deflateInit2 returned error:%d:%s\n"),
						 err, pz->msg);
			return -1;
		}
	...
	if (zlibLevel != m_zsLevel[streamId]) {
		rdlog.Print(LL_INTINFO,
					 rdLOG("calling deflateParams with zlib level:%d\n"),
					 zlibLevel);
		int err = deflateParams (pz, zlibLevel, zlibStrategy);
		if (err != Z_OK) {
			rdlog.Print(LL_INTINFO,
						 rdLOG("deflateParams returned error:%d:%s\n"),
						 err, pz->msg);
			return -1;
		}
		m_zsLevel[streamId] = zlibLevel;
	...
	if ( deflate (pz, Z_SYNC_FLUSH) != Z_OK ||
		...
		rdlog.Print(LL_INTINFO, rdLOG("deflate() call failed.\n"));
		return -1;
	}
...
int
rdEncodeTight::SendCompressedData(int compressedLen)
{
...
void
rdEncodeTight::FillPalette8(int count)
{
...
#define DEFINE_FILL_PALETTE_FUNCTION(bpp)									  \
																			  \
void																		  \
rdEncodeTight::FillPalette##bpp(int count) 								  \
{																			  \
...
void
rdEncodeTight::PaletteReset(void)
{
	m_paletteNumColors = 0;
...
int
rdEncodeTight::PaletteInsert(CARD32 rgb, int numPixels, int bpp)
{
	COLOR_LIST *pnode;
...
void
rdEncodeTight::Pack24(BYTE *buf, int count)
{
	CARD32 *buf32;
	CARD32 pix;
...
#define DEFINE_IDX_ENCODE_FUNCTION(bpp) 									  \
																			  \
void																		  \
rdEncodeTight::EncodeIndexedRect##bpp(BYTE *buf, int count)				  \
{																			  \
	COLOR_LIST *pnode;														  \
	CARD##bpp *src; 														  \
	CARD##bpp rgb;															  \
...
#define DEFINE_MONO_ENCODE_FUNCTION(bpp)									  \
																			  \
void																		  \
rdEncodeTight::EncodeMonoRect##bpp(BYTE *buf, int w, int h)				  \
{																			  \
	CARD##bpp *ptr; 														  \
	CARD##bpp bg;															  \
...
void
rdEncodeTight::FilterGradient24(BYTE *buf, int w, int h)
{
	CARD32 *buf32;
	CARD32 pix32;
	int *prevRowPtr;
...
#define DEFINE_GRADIENT_FILTER_FUNCTION(bpp)								  \
																			  \
void																		  \
rdEncodeTight::FilterGradient##bpp(CARD##bpp *buf, int w, int h)			  \
{																			  \
	CARD##bpp pix, diff;													  \
	bool endianMismatch;													  \
	int *prevRowPtr;														  \
...
int
rdEncodeTight::DetectSmoothImage (int w, int h)
{
...
unsigned long
rdEncodeTight::DetectSmoothImage24 (int w, int h)
{
	int diffStat[256];
	int pixelCount = 0;
	int pix, left[3];
...
#define DEFINE_DETECT_FUNCTION(bpp) 										  \
																			  \
unsigned long																  \
rdEncodeTight::DetectSmoothImage##bpp (int w, int h)						  \
{																			  \
	bool endianMismatch;													  \
	CARD##bpp pix;															  \
	int maxColor[3], shiftBits[3];											  \
...
int
rdEncodeTight::SendJpegRect(BYTE *dst, int w, int h, int quality)
{
...
void
rdEncodeTight::PrepareRowForJpeg(BYTE *dst, int y, int w)
{
	if (m_remoteformat.bitsPerPixel == 32) {
...
void
rdEncodeTight::PrepareRowForJpeg24(BYTE *dst, CARD32 *src, int count)
{
	int r_shift, g_shift, b_shift;
	if (!m_localformat.bigEndian == !m_remoteformat.bigEndian) {
		r_shift = m_remoteformat.redShift;
...
#define DEFINE_JPEG_GET_ROW_FUNCTION(bpp)									\
																			\
void																		\
rdEncodeTight::PrepareRowForJpeg##bpp(BYTE *dst, CARD##bpp *src, int count)\
{																			\
	bool endianMismatch =													\
