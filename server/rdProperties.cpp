...
#include "rdAdvancedProperties.h"
#include "rdServer.h"
#include "rdPasswd.h"

const char WINrd_REGISTRY_KEY [] = "Software\\ORL\\Winrd3";
const char NO_PASSWORD_WARN [] = "WARNING : Running Winrd without setting a password is "
								"a dangerous security risk!\n"
								"Until you set a password, Winrd will not accept incoming connections.";
const char NO_OVERRIDE_ERR [] = "This machine has been preconfigured with Winrd settings, "
...
const char NO_PASSWD_NO_OVERRIDE_ERR [] =
								...
								"You must contact a System Administrator to configure Winrd properly.";
...
const char NO_PASSWD_NO_LOGON_WARN [] =
								"WARNING : This machine has no default password set.  Winrd will present the "
								"Default Properties dialog now to allow one to be entered.";
const char NO_CURRENT_USER_ERR [] = "The Winrd settings for the current user are unavailable at present.";
const char CANNOT_EDIT_DEFAULT_PREFS [] = "You do not have sufficient priviliges to edit the default local Winrd settings.";

// Constructor & Destructor
rdProperties::rdProperties()
{
...
// Initialisation
BOOL
rdProperties::Init(rdServer *server)
{
	    ...
	    if (strlen(plain) == 0)
			if (!m_allowproperties) {
				...
				if (m_server->AuthRequired()) {
					...
					if (!rdService::CurrentUser(username, sizeof(username)))
						return FALSE;
...
void
rdProperties::Show(BOOL show, BOOL usersettings)
{
	if (show)
	{
		m_inadvanced = FALSE;
		if (!m_allowproperties)
		{
			...
			MessageBox(NULL, NO_OVERRIDE_ERR, "Winrd Error", MB_OK | MB_ICONEXCLAMATION);
			return;
		}
		...
		if (usersettings) {
			...
			if (!rdService::CurrentUser(username, sizeof(username)))
				return;
			if (strcmp(username, "") == 0) {
				MessageBox(NULL, NO_CURRENT_USER_ERR, "Winrd Error", MB_OK | MB_ICONEXCLAMATION);
				return;
			}
		} else {
			...
			if (!canEditDefaultPrefs) {
				MessageBox(NULL, CANNOT_EDIT_DEFAULT_PREFS, "Winrd Error", MB_OK | MB_ICONEXCLAMATION);
				return;
			}
		}
		...
		{
			if (usersettings)
				rdlog.Print(LL_INTINFO, rdLOG("show per-user Properties\n"));
			else
				rdlog.Print(LL_INTINFO, rdLOG("show default system Properties\n"));
			...
			for (;;)
			{
				...
				rdlog.Print(LL_INTINFO, rdLOG("dialog result = %d\n"), result);
				...
				{
				    rdPasswd::ToText plain(passwd);
					rdPasswd::ToText plain_viewonly(passwd_viewonly);
				...
				rdlog.Print(LL_INTERR, rdLOG("warning - empty password\n"));

				// The password is empty, so if OK was used then redisplay the box,
				// otherwise, if CANCEL was used, close down Winrd
				if (result == IDCANCEL)
				{
				    rdlog.Print(LL_INTERR, rdLOG("no password - QUITTING\n"));
				    PostQuitMessage(0);
				    return;
				}
				...
				int result2 = MessageBox(NULL, NO_PASSWORD_WARN,
				    "Winrd Warning", MB_OK | MB_ICONEXCLAMATION);
...
BOOL CALLBACK
rdProperties::DialogProc(HWND hwnd,
						  UINT uMsg,
						  WPARAM wParam,
						  LPARAM lParam )
{
	...
	rdProperties *_this = (rdProperties *) GetWindowLong(hwnd, GWL_USERDATA);
	...
	case WM_INITDIALOG:
		{
			...
			_this = (rdProperties *) lParam;
			_this->m_dlgvisible = TRUE;

			// Set the dialog box's title to indicate which Properties we're editting
			if (_this->m_usersettings) {
				SetWindowText(hwnd, "Winrd: Current User Properties");
			} else {
				SetWindowText(hwnd, "Winrd: Default Local System Properties");
			}
	...
	case WM_COMMAND:
		...
		case IDC_APPLY:
			{
				...
				if (strcmp(passwd, "~~~~~~~~") != 0) {
					if (len == 0) {
						rdPasswd::FromClear crypt;
						_this->m_server->SetPassword(crypt);
					} else {
						rdPasswd::FromText crypt(passwd);
						_this->m_server->SetPassword(crypt);
					}
				}
				...
				if (LOWORD(wParam) == IDOK)
				{
					...
					rdlog.Print(LL_INTINFO, rdLOG("enddialog (OK)\n"));
		...
		case IDCANCEL:
			rdlog.Print(LL_INTINFO, rdLOG("enddialog (CANCEL)\n"));
		...
		case IDADVANCED:
			rdlog.Print(LL_INTINFO, rdLOG("newdialog (ADVANCED)\n"));
			{
				EnableWindow(hwnd, FALSE);
				_this->m_inadvanced = TRUE;
				rdAdvancedProperties *aprop = new rdAdvancedProperties();
...
void
rdProperties::InitPortSettings(HWND hwnd)
{
...
LONG
rdProperties::LoadInt(HKEY key, LPCSTR valname, LONG defval)
{
	LONG pref;
...
void
rdProperties::LoadPassword(HKEY key, char *buffer, const char *entry_name)
{
...
char *
rdProperties::LoadString(HKEY key, LPCSTR keyname)
{
	DWORD type = REG_SZ;
	DWORD buflen = 0;
	BYTE *buffer = 0;
...
void
rdProperties::Load(BOOL usersettings)
{
	if (m_dlgvisible) {
		rdlog.Print(LL_INTWARN, rdLOG("service helper invoked while Properties panel displayed\n"));
		return;
	}
	...
	// 1.	Look in HKEY_LOCAL_MACHINE/Software/ORL/Winrd3/%username%
	...
	//		HKEY_CURRENT_USER/Software/ORL/Winrd3

	// GET THE CORRECT KEY TO READ FROM

	// Get the user name / service name
	if (!rdService::CurrentUser((char *)&username, sizeof(username)))
		return;
	...
	// Try to get the machine registry key for Winrd
	if (RegCreateKeyEx(HKEY_LOCAL_MACHINE,
		WINrd_REGISTRY_KEY,
		0, REG_NONE, REG_OPTION_NON_VOLATILE,
		KEY_READ, NULL, &hkLocal, &dw) != ERROR_SUCCESS)
	...
	rdlog.Print(LL_INTINFO, rdLOG("loading local-only settings\n"));
	rdlog.SetMode(LoadInt(hkLocal, "DebugMode", 0));
	rdlog.SetLevel(LoadInt(hkLocal, "DebugLevel", 0));
	...
	// NOTE: Realrd sets ConnectPriority to 0 by default, we set it to 2.
	...
	rdlog.Print(LL_INTINFO, rdLOG("clearing user settings\n"));
	...
	if (hkDefault != NULL)
	{
		rdlog.Print(LL_INTINFO, rdLOG("loading DEFAULT local settings\n"));
	...
	if (usersettings) {
		...
		if (hkLocalUser != NULL)
		{
			rdlog.Print(LL_INTINFO, rdLOG("loading \"%s\" local settings\n"), username);
		...
		if (m_allowproperties && (strcmp(username, "SYSTEM") != 0))
		{
			...
			{
				rdlog.Print(LL_INTINFO, rdLOG("loading \"%s\" global settings\n"), username);
		...
		rdlog.Print(LL_INTINFO, rdLOG("bypassing user-specific settings (both local and global)\n"));
	}
...
void
rdProperties::LoadUserPrefs(HKEY appkey)
{
...
void
rdProperties::SaveInt(HKEY key, LPCSTR valname, LONG val)
{
	RegSetValueEx(key, valname, 0, REG_DWORD, (LPBYTE) &val, sizeof(val));
}

void
rdProperties::SavePassword(HKEY key, const char *buffer, const char *entry_name)
{
...
rdProperties::Save()
{
	...
	if (m_usersettings) {
		...
		// Try to get the per-user, global registry key for Winrd
		if (RegCreateKeyEx(HKEY_CURRENT_USER,
			WINrd_REGISTRY_KEY,
			0, REG_NONE, REG_OPTION_NON_VOLATILE,
		...
		// Try to get the default local registry key for Winrd
		HKEY hkLocal;
		if (RegCreateKeyEx(HKEY_LOCAL_MACHINE,
			WINrd_REGISTRY_KEY,
			0, REG_NONE, REG_OPTION_NON_VOLATILE,
			KEY_READ, NULL, &hkLocal, &dw) != ERROR_SUCCESS) {
			MessageBox(NULL, "MB1", "Wrd", MB_OK);
			return;
		}
		if (RegCreateKeyEx(hkLocal,
			"Default",
...
void
rdProperties::SaveUserPrefs(HKEY appkey)
{
	// SAVE THE PER USER PREFS
	rdlog.Print(LL_INTINFO, rdLOG("saving current settings to registry\n"));
	...
	// Connection querying settings
	// FIXME: Currently query settings are saved in rdAdvancedProperties,
