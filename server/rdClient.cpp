...
// rdClient thread class

class rdClientThread : public omni_thread
{
public:

	// Init
	virtual BOOL Init(rdClient *client,
		rdServer *server,
		VSocket *socket,
		BOOL auth,
		BOOL shared);
...
BOOL
rdClientThread::Init(rdClient *client, rdServer *server, VSocket *socket, BOOL auth, BOOL shared)
{
...
rdClientThread::InitVersion()
{
	...
	if (major != rfbProtocolMajorVersion) {
		rdlog.Print(LL_CONNERR, rdLOG("protocol versions do not match\n"));
		return FALSE;
	}
...
rdClientThread::InitAuthenticate()
{
	...
	if (!m_server->LoopbackOk())
	{
		...
		if ((localname != NULL) && (remotename != NULL))
		{
			...
			if (!ok)
			{
				rdlog.Print(LL_CONNERR, rdLOG("loopback connection attempted - client rejected\n"));
	...
	rdServer::AcceptQueryReject verified;
	if (m_auth) {
		verified = rdServer::aqrAccept;
	} else {
	...
	if (verified == rdServer::aqrQuery) {
		rdAcceptDialog *acceptDlg =
			new rdAcceptDialog(m_server->QueryTimeout(),
	...
	if (verified == rdServer::aqrReject) {
		rdlog.Print(LL_CONNERR, rdLOG("Client connection rejected\n"));
		CARD32 auth_val = Swap32IfLE(rfbConnFailed);
	...
	else
	{
		...
		CARD32 auth_val = Swap32IfLE(rfbrdAuth);
		if (!m_socket->SendExact((char *)&auth_val, sizeof(auth_val)))
			return FALSE;
		...
		BOOL auth_ok = FALSE;
		{
			...
			rdEncryptBytes((BYTE *)&challenge, plain);
			...
			if (memcmp(challenge, response, sizeof(response)) == 0) {
				...
				rdEncryptBytes((BYTE *)&challenge_viewonly, plain_viewonly);
		...
		if (!auth_ok)
		{
			rdlog.Print(LL_CONNERR, rdLOG("authentication failed\n"));

			authmsg = Swap32IfLE(rfbrdAuthFailed);
		...
		else
		{
			...
			authmsg = Swap32IfLE(rfbrdAuthOK);
	...
	if (!client_ini.shared && !m_shared)
	{
		...
		{
			// Incoming
			rdlog.Print(LL_INTINFO, rdLOG("non-shared connection - disconnecting old clients\n"));
		...
		} else if (m_server->ConnectPriority() > 1)
		{
			...
			{
				rdlog.Print(LL_CLIENTS, rdLOG("connections already exist - client rejected\n"));
				return FALSE;
			}
		}
	}
...
{
	// This routine is used by the rd client handler to clear the
...
rdClientThread::run(void *arg)
{
	...
	rdlog.Print(LL_CLIENTS, rdLOG("client connected : %s (id %hd)\n"),
	...
	if (!m_socket->SetTimeout(30000))
		rdlog.Print(LL_INTERR, rdLOG("failed to set socket timeout, error=%d\n"), GetLastError());
	...
	rdlog.Print(LL_INTINFO, rdLOG("negotiated protocol version\n"));
	...
	rdlog.Print(LL_INTINFO, rdLOG("authenticated connection\n"));
	...
	rdlog.Print(LL_INTINFO, rdLOG("sent pixel format to client\n"));
	...
	while (connected)
	{
		...
		if (!rdService::InputDesktopSelected())
			if (!rdService::SelectDesktop(NULL))
				break;
		...
		case rfbSetPixelFormat:
			...
			{	omni_mutex_lock l(m_client->m_regionLock);
				...
				{
					rdlog.Print(LL_CONNERR, rdLOG("remote pixel format invalid\n"));
			...
			msg.se.nEncodings = Swap16IfLE(msg.se.nEncodings);
			{
				...
				{ omni_mutex_lock l(m_client->m_regionLock);
					...
					if (Swap32IfLE(encoding) == rfbEncodingXCursor) {
						...
						rdlog.Print(LL_INTINFO, rdLOG("X-style cursor shape updates enabled\n"));
						continue;
					}
					...
					if (Swap32IfLE(encoding) == rfbEncodingRichCursor) {
						...
						shapeupdates_requested = TRUE;
						rdlog.Print(LL_INTINFO, rdLOG("Full-color cursor shape updates enabled\n"));
						continue;
					}
						...
						(Swap32IfLE(encoding) <= rfbEncodingCompressLevel9))
					{
						...
						rdlog.Print(LL_INTINFO, rdLOG("compression level requested: %d\n"), level);
						continue;
					}
						...
						(Swap32IfLE(encoding) <= rfbEncodingQualityLevel9))
					{
						...
						rdlog.Print(LL_INTINFO, rdLOG("image quality level requested: %d\n"), level);
						continue;
					}
					...
					if (Swap32IfLE(encoding) == rfbEncodingLastRect) {
						...
						rdlog.Print(LL_INTINFO, rdLOG("LastRect protocol extension enabled\n"));
						continue;
					}
				...
				if (shapeupdates_requested && pointerpos_requested) {
					...
					rdlog.Print(LL_INTINFO, rdLOG("PointerPos protocol extension enabled\n"));
				}
				...
				if (!encoding_set)
				{
					...
					rdlog.Print(LL_INTINFO, rdLOG("defaulting to raw encoder\n"));

					if (!m_client->m_buffer->SetEncoding(Swap32IfLE(rfbEncodingRaw)))
					{
						rdlog.Print(LL_INTERR, rdLOG("failed to select raw encoder!\n"));
		...
		case rfbKeyEvent:
			...
			{				
				if (m_client->m_keyboardenabled)
				{
					...
					rdKeymap::keyEvent(msg.ke.key, msg.ke.down != 0);
	...
	// associated client.
	rdlog.Print(LL_CLIENTS, rdLOG("client disconnected : %s (id %hd)\n"),
				 m_client->GetClientName(), m_client->GetClientId());
...
rdClient::rdClient()
{
	rdlog.Print(LL_INTINFO, rdLOG("rdClient() executing...\n"));
...
rdClient::~rdClient()
{
	...
	if (m_socket != NULL)
	{
		rdlog.Print(LL_INTINFO, rdLOG("deleting socket\n"));
	...
	if (m_buffer != NULL)
	{
		rdlog.Print(LL_INTINFO, rdLOG("deleting buffer\n"));
				...
				rdClientId newid)
{
	...
	m_thread = new rdClientThread;
	if (m_thread == NULL)
		return FALSE;
	return ((rdClientThread *)m_thread)->Init(this, m_server, m_socket, auth, shared);
...
void
rdClient::SetBuffer(rdBuffer *buffer)
{
...
void
rdClient::UpdateRegion(rdRegion &region)
{
...
void
rdClient::CopyRect(RECT &dest, POINT &source)
{
...
void
rdClient::UpdateClipText(LPSTR text)
{
...
// Internal methods
BOOL
rdClient::SendRFBMsg(CARD8 type, BYTE *buffer, int buflen)
{
	...
	{
		rdlog.Print(LL_CONNERR, rdLOG("failed to send RFB message to client\n"));
...
inline void
rdClient::CheckRects(rdRegion &rgn, rectlist &rects)
{
	rectlist::iterator i;
...
inline void
rdClient::ClearRects(rdRegion &rgn, rectlist &rects)
{
	rectlist::iterator i;
...
inline void
rdClient::GrabRegion(rdRegion &rgn)
{
	rectlist::iterator i;
	rectlist rects;
	RECT grabRect;
...
rdClient::SendUpdate()
{
	rdRegion toBeSent;			// Region to actually be sent
	rectlist toBeSentList;		// List of rectangles to actually send
	rdRegion toBeDone;			// Region to check
...
BOOL
rdClient::SendRectangles(rectlist &rects)
{
	RECT rect;
...
BOOL
rdClient::SendRectangle(RECT &rect)
{
...
BOOL
rdClient::SendCopyRect(RECT &dest, POINT &source)
{
...
BOOL
rdClient::SendCursorShapeUpdate()
{
	m_cursor_update_pending = FALSE;
