...
// Constants
const UINT RFB_SCREEN_UPDATE = RegisterWindowMessage("Winrd.Update.DrawRect");
const UINT RFB_COPYRECT_UPDATE = RegisterWindowMessage("Winrd.Update.CopyRect");
const UINT RFB_MOUSE_UPDATE = RegisterWindowMessage("Winrd.Update.Mouse");
const char szDesktopSink[] = "Winrd desktop sink";
...
// This handles the messages posted by RFBLib to the rdDesktop window

class rdDesktopThread : public omni_thread
{
public:
	rdDesktopThread() {m_returnsig = NULL;};
protected:
	~rdDesktopThread() {if (m_returnsig != NULL) delete m_returnsig;};
public:
	virtual BOOL Init(rdDesktop *desktop, rdServer *server);
...
BOOL
rdDesktopThread::Init(rdDesktop *desktop, rdServer *server)
{
...
void
rdDesktopThread::ReturnVal(BOOL result)
{
	omni_mutex_lock l(m_returnLock);
...
void *
rdDesktopThread::run_undetached(void *arg)
{
	...
	{
		rdService::SelectHDESK(home_desktop);
		ReturnVal(FALSE);
		return NULL;
	}
	...
	while (TRUE)
	{
		...
		if (!PeekMessage(&msg, m_desktop->Window(), NULL, NULL, PM_NOREMOVE))
		{
			...
			if (m_desktop->m_displaychanged || !rdService::InputDesktopSelected())
			{
				rdlog.Print(LL_STATE, rdLOG("display resolution or desktop changed.\n"));
				...
				{
					rdlog.Print(LL_INTERR, rdLOG("failed to close desktop server.\n"));
				...
				if (!m_desktop->Startup())
				{
					rdlog.Print(LL_INTERR, rdLOG("failed to re-start desktop server.\n"));
					m_server->KillAuthClients();
					break;
				}
				...
				{
					rdlog.Print(LL_CONNERR, rdLOG("screen format has changed - disconnecting clients.\n"));
					m_server->KillAuthClients();
					break;
				}
		...
		if (msg.message == RFB_SCREEN_UPDATE)
		{
			...
			if ((rect.left < 0) || (rect.top < 0) ||
				...
				rdlog.Print(50, rdLOG("update:%d,%dx%d,%d\n"),
...
//			m_server->UpdateRect(rect);
	...
	rdlog.Print(LL_INTINFO, rdLOG("quitting desktop server thread\n"));
...
rdDesktop::~rdDesktop()
{
	rdlog.Print(LL_INTINFO, rdLOG("killing desktop server\n"));
...
rdDesktop::Startup()
{
	...
	// Configure the display for optimal rd performance.
	...
	{
		rdlog.Print(LL_INTERR, rdLOG("failed to set system hooks\n"));
	...
	if ((rd_WINDOWPOS_ATOM = GlobalAddAtom(rd_WINDOWPOS_ATOMNAME)) == 0) {
		rdlog.Print(LL_INTERR, rdLOG("GlobalAddAtom() failed.\n"));
		return FALSE;
	}
...
rdDesktop::Shutdown()
{
	...
	if (m_hrootdc != NULL)
	{
		...
		{
			rdlog.Print(LL_INTERR, rdLOG("failed to ReleaseDC\n"));
		}
		m_hrootdc = NULL;
	}
	if (m_hmemdc != NULL)
	{
		...
		{
			rdlog.Print(LL_INTERR, rdLOG("failed to DeleteDC\n"));
		}
		m_hmemdc = NULL;
	}
	if (m_membitmap != NULL)
	{
		...
		{
			rdlog.Print(LL_INTERR, rdLOG("failed to DeleteObject\n"));
		}
		m_membitmap = NULL;
	}
	...
	if (rd_WINDOWPOS_ATOM != NULL) {
		if (GlobalDeleteAtom(rd_WINDOWPOS_ATOM) != 0) {
			rdlog.Print(LL_INTERR, rdLOG("failed to delete atom!\n"));
		}
	}
...
BOOL
rdDesktop::InitDesktop()
{
	if (rdService::InputDesktopSelected())
		return TRUE;

	// Ask for the current input desktop
	return rdService::SelectDesktop(NULL);
}
...
rdDesktop::KillScreenSaver()
{
	...
	rdlog.Print(LL_INTINFO, rdLOG("KillScreenSaver...\n"));
	...
	case VER_PLATFORM_WIN32_NT:
		{
			...
			if (hDesk != NULL)
			{
				rdlog.Print(LL_INTINFO, rdLOG("Killing ScreenSaver\n"));
...
void rdDesktop::ChangeResNow()
{
	...
	if (! EnumDisplaySettings( 0, ENUM_CURRENT_SETTINGS, lpDevMode)) {
		...
		rdlog.Print(LL_INTINFO, rdLOG("SCR-WBB: could not get "
	...
	rdlog.Print(LL_INTINFO, rdLOG("SCR-WBB: current display: "
	...
	if ((valType == REG_SZ) &&
		...
		if ((valType == REG_SZ ) &&
			...
			rdlog.Print(LL_INTINFO, rdLOG("SCR-WBB: attempting to change "
											"resolution w=%d h=%d\n"),
...
// *** First the HKLM\\Software\\ORL\\Winrd\\<regName> string is checkted
// *** for presence, string type and Yes/yes/Y/y, if so, SystemParameterInfo
// *** is called.
void
rdDesktop::DisableIfRegSystemParameter(char *regName,
										int spiCommand,
										int spiParamInt,
										void* spiParamPtr,
										int spiUpdate)
{
	...
	// *** This is placed here rather than rdHooks.cpp as it causes
	// *** rd to crash if put there. - Jeremy Peaks
	...
	if ((valType == REG_SZ) &&
		...
		rdlog.Print(LL_INTINFO, rdLOG("SCR-WBB: attempting to %s.\n"), regName);
...
void
rdDesktop::SetupDisplayForConnection()
{
...
BOOL
rdDesktop::OptimizeDisplayForConnection()
{
	...
	if ((valType == REG_SZ) &&
		...
		rdlog.Print(LL_INTINFO, rdLOG("SCR-WBB: desktop optimization enabled.\n"));
		result = TRUE;

		rdlog.Print(LL_INTINFO, rdLOG("SCR-WBB: attempting to disable pattern.\n"));
		SystemParametersInfo(SPI_SETDESKPATTERN, 0, 0, SPIF_SENDCHANGE);

		rdlog.Print(LL_INTINFO, rdLOG("SCR-WBB: attempting to disable wallpaper.\n"));
		SystemParametersInfo(SPI_SETDESKWALLPAPER, 0, "", SPIF_SENDCHANGE);

		rdlog.Print(LL_INTINFO, rdLOG("SCR-WBB: attempting to disable font smoothing.\n"));
		SystemParametersInfo(SPI_SETFONTSMOOTHING, FALSE, 0, SPIF_SENDCHANGE);

		rdlog.Print(LL_INTINFO, rdLOG("SCR-WBB: attempting to disable full window drags.\n"));
...
void
rdDesktop::ResetDisplayToNormal()
{
...
rdDesktop::InitBitmap()
{
	...
	rdlog.Print(LL_INTINFO, rdLOG("bitmap dimensions are %d x %d\n"),
				 m_bmrect.right, m_bmrect.bottom);

	// Create a compatible memory DC
	m_hmemdc = CreateCompatibleDC(m_hrootdc);
	if (m_hmemdc == NULL) {
		rdlog.Print(LL_INTERR, rdLOG("CreateCompatibleDC() failed, error=%d\n"),
					 GetLastError());
		return FALSE;
	}
	...
	{
		rdlog.Print(LL_INTERR, rdLOG("BitBlt() is not supported\n"));
		MessageBox(
			NULL,
			"rdDesktop : root device doesn't support BitBlt\n"
			"Winrd cannot be used with this graphic device driver",
			szAppName,
			MB_ICONSTOP | MB_OK
			);
		return FALSE;
	...
	{
		rdlog.Print(LL_INTERR, rdLOG("GetDIBits() is not supported\n"));
		MessageBox(
			NULL,
			"rdDesktop : memory device doesn't support GetDIBits\n"
			"Winrd cannot be used with this graphics device driver",
			szAppName,
			MB_ICONSTOP | MB_OK
			);
		return FALSE;
	...
	if (m_membitmap == NULL) {
		rdlog.Print(LL_INTERR, rdLOG("failed to create memory bitmap, error=%d\n"),
					 GetLastError());
		return FALSE;
	}
	rdlog.Print(LL_INTINFO, rdLOG("created memory bitmap\n"));
	...
	if (::GetDIBits(m_hmemdc, m_membitmap, 0, 1, NULL,
		...
		rdlog.Print(LL_INTERR, rdLOG("GetDIBits() failed, call #1\n"));
		return FALSE;
	}
	if (::GetDIBits(m_hmemdc, m_membitmap, 0, 1, NULL,
					&m_bminfo.bmi, DIB_RGB_COLORS) == 0) {
		rdlog.Print(LL_INTERR, rdLOG("GetDIBits() failed, call #2\n"));
		return FALSE;
	}
	rdlog.Print(LL_INTINFO, rdLOG("got bitmap format\n"));
	rdlog.Print(LL_INTINFO, rdLOG("DBG:display context has %d planes!\n"), GetDeviceCaps(m_hrootdc, PLANES));
	rdlog.Print(LL_INTINFO, rdLOG("DBG:memory context has %d planes!\n"), GetDeviceCaps(m_hmemdc, PLANES));
	if (GetDeviceCaps(m_hmemdc, PLANES) != 1)
	{
		rdlog.Print(LL_INTERR, rdLOG("number of planes in memory context is more than 1\n"));
		MessageBox(
			NULL,
			"rdDesktop : current display is PLANAR, not CHUNKY!\n"
			"Winrd cannot be used with this graphics device driver",
			szAppName,
			MB_ICONSTOP | MB_OK
			);
...
rdDesktop::ThunkBitmapInfo()
{
	...
	case 1:
	case 4:
		rdlog.Print(LL_INTINFO, rdLOG("DBG:used/bits/planes/comp/size "
										"= %d/%d/%d/%d/%d\n"),
		...
		// Display format is non-rd compatible - use the slow blit method
		m_formatmunged = TRUE;
		break;	
	case 24:
...
rdDesktop::SetPixShifts()
{
	...
	default:
		...
		if (m_bminfo.truecolour)
		{
			rdlog.Print(LL_INTERR, "unsupported truecolour pixel format for SetPixShifts()\n");
			return FALSE;
		}
		rdlog.Print(LL_INTINFO, rdLOG("DBG:palette-based desktop in SetPixShifts()\n"));
		return TRUE;
	}
	...
	rdlog.Print(LL_INTINFO, rdLOG("DBG:true-color desktop in SetPixShifts()\n"));
	return TRUE;
}

BOOL
rdDesktop::SetPalette()
{
	...
	if (!m_bminfo.truecolour)
	{
		...
		if (palette == NULL) {
			rdlog.Print(LL_INTERR, rdLOG("error allocating palette\n"));
			return FALSE;
		}
		...
		{
			rdlog.Print(LL_INTERR, rdLOG("GetSystemPaletteEntries() failed.\n"));
			delete [] palette;
			return FALSE;
		}
		...
		if (pal == NULL)
		{
			rdlog.Print(LL_INTERR, rdLOG("CreatePalette() failed.\n"));
			delete [] palette;
			return FALSE;
		}
		...
		if (oldpalette == NULL)
		{
			rdlog.Print(LL_INTERR, rdLOG("SelectPalette() failed.\n"));
			delete [] palette;
			DeleteObject(pal);
			return FALSE;
		}
		...
		if (RealizePalette(m_hmemdc) == GDI_ERROR)
			rdlog.Print(LL_INTWARN, rdLOG("warning - failed to RealizePalette\n"));

		// It worked!
		delete [] palette;
		DeleteObject(oldpalette);

		rdlog.Print(LL_INTINFO, rdLOG("initialised palette OK\n"));
		return TRUE;
	}

	// Not a palette based local screen - forget it!
	rdlog.Print(LL_INTINFO, rdLOG("no palette data for truecolour display\n"));
	return TRUE;
}
...
rdDesktop::InitWindow()
{
	...
	if (m_hwnd == NULL) {
		rdlog.Print(LL_INTERR, rdLOG("CreateWindow() failed.\n"));
		return FALSE;
	}
...
void
rdDesktop::EnableOptimisedBlits()
{
	rdlog.Print(LL_INTINFO, rdLOG("attempting to enable DIBsection blits\n"));

	if (m_formatmunged) {
		rdlog.Print(LL_INTINFO, rdLOG("unable to enable fast blits\n"));
		return;
	}
...
/*
	...
	if (!m_bminfo.truecolour) {
		...
		palette = (LOGPALETTE *) new char[size];
		if (palette == NULL) {
			rdlog.Print(LL_INTERR, rdLOG("failed to create temp fast palette - disabling fast blits\n"));
			return;
		}
		...
		if (GetSystemPaletteEntries(m_hrootdc,
			...
			delete [] palette;
			rdlog.Print(LL_INTERR, rdLOG("failed to init fast palette - disabling fast blits\n"));
			return;
		}
		...
		rdlog.Print(LL_INTINFO, rdLOG("initialised fast palette OK\n"));
	}
*/
	...
	if (tempbitmap == NULL) {
		rdlog.Print(LL_INTWARN, rdLOG("failed to build DIB section - reverting to slow blits\n"));
		m_DIBbits = NULL;
		return;
	}
	...
	rdlog.Print(LL_INTINFO, rdLOG("enabled fast DIBsection blits OK\n"));
}

BOOL
rdDesktop::Init(rdServer *server)
{
	rdlog.Print(LL_INTINFO, rdLOG("initialising desktop server\n"));
	...
	rdDesktopThread *thread = new rdDesktopThread;
	if (thread == NULL)
		return FALSE;
	m_thread = thread;
...
void
rdDesktop::RequestUpdate()
{
	PostMessage(m_hwnd, WM_TIMER, 0, 0);
}

int
rdDesktop::ScreenBuffSize()
{
...
void
rdDesktop::FillDisplayInfo(rfbServerInitMsg *scrinfo)
{
...
// an update.
void
rdDesktop::CaptureScreen(RECT &rect, BYTE *scrBuff, UINT scrBuffSize)
{
...
void
rdDesktop::CaptureMouse(BYTE *scrBuff, UINT scrBuffSize)
{
	// Protect the memory bitmap
	omni_mutex_lock l(m_bitbltlock);
...
BOOL
rdDesktop::GetRichCursorData(BYTE *databuf, HCURSOR hcursor, int width, int height)
{
...
void
rdDesktop::SetCursor(HCURSOR cursor)
{
	if (cursor == NULL)
		m_hcursor = m_hdefcursor;
	else
...
void
rdDesktop::SetClipText(LPSTR text)
{
...
inline void
rdDesktop::MaskToMaxAndShift(DWORD mask, CARD16 &max, CARD8 &shift)
{
...
void
rdDesktop::CopyToBuffer(RECT &rect, BYTE *destbuff, UINT destbuffsize)
{
	...
	if (m_DIBbits == NULL) {
		...
		{
#ifdef _MSC_VER
			_RPT1(_CRT_WARN, "rdDesktop : [1] GetDIBits failed! %d\n", GetLastError());
			_RPT3(_CRT_WARN, "rdDesktop : thread = %d, DC = %d, bitmap = %d\n", omni_thread::self(), m_hmemdc, m_membitmap);
			_RPT2(_CRT_WARN, "rdDesktop : y = %d, height = %d\n", y_inv, (rect.bottom-rect.top));
#endif
		}
	} else {
...
{
	HANDLE prop = GetProp(hwnd, (LPCTSTR) MAKELONG(rd_WINDOWPOS_ATOM, 0));
	if (prop != NULL)
	{
		if (IsWindowVisible(hwnd))
		{
			...
			if (GetWindowRect(hwnd, &dest))
			{
				...
				if ((source.x != dest.left) || (source.y != dest.top))
				{
					...
					SetProp(hwnd,
						(LPCTSTR) MAKELONG(rd_WINDOWPOS_ATOM, 0),
						(HANDLE) MAKELONG(x, y));

					// Notify all clients of the copyrect
					((rdServer*)arg)->CopyRect(dest, source);

					// Get the client to check for damage
					((rdServer*)arg)->UpdateRect(dest);
				}
			}
			else
				RemoveProp(hwnd, (LPCTSTR) MAKELONG(rd_WINDOWPOS_ATOM, 0));
		}
		else
		{
			RemoveProp(hwnd, (LPCTSTR) MAKELONG(rd_WINDOWPOS_ATOM, 0));
...
{
	rdDesktop *_this = (rdDesktop*)GetWindowLong(hwnd, GWL_USERDATA);
