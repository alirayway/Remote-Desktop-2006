...
// TO WORK FOR WINrd.  THE PALETTE SHOULD REALLY BE RETRIEVED
// FROM THE rdDESKTOP OBJECT, RATHER THAN FROM THE OS DIRECTLY
...
rfbInitColourMapSingleTableOUT (char **table,
								...
								rfbPixelFormat *out)
{
	rdlog.Print(LL_ALL, rdLOG("rfbInitColourMapSingleTable called\n"));
    ...
    *table = (char *)malloc(nEntries * sizeof(OUT_T));
	if (*table == NULL)
	{
		rdlog.Print(LL_INTERR, rdLOG("failed to allocate translation table\n"));
		return;
	}
	...
	if (GetSystemPaletteEntries(hDC,
		0, 256, palette) == 0)
	{
		rdlog.Print(LL_INTERR, rdLOG("failed to get system palette, error=%d\n"),
					 GetLastError());
		ReleaseDC(NULL, hDC);
		return;
	}
	...
	rdlog.Print(LL_ALL, rdLOG("rfbInitColourMapSingleTable done\n"));
}
