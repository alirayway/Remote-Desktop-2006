...
// This file implements the rdEncoder-derived rdEncodeRRE class.
...
UINT
rdEncodeRRE::RequiredBuffSize(UINT width, UINT height)
{
	return rdEncoder::RequiredBuffSize(width, height);
}

UINT
rdEncodeRRE::NumCodedRects(RECT &rect)
{
	return 1;
}
...
inline UINT
rdEncodeRRE::EncodeRect(BYTE *source, BYTE *dest, const RECT &rect)
{
	int subrects = -1;
