...
// This file implements the rdEncoder-derived rdEncodeCoRRE class.
...
void rdEncodeCoRRE::Init()
{
	rdEncoder::Init();
}

UINT rdEncodeCoRRE::RequiredBuffSize(UINT width, UINT height)
{
	RECT fullscreen;
	UINT codedrects;
...
UINT
rdEncodeCoRRE::NumCodedRects(RECT &rect)
{
...
/*
 * This version modified for Winrd by jnw.
 */
...
/*
 * rdEncodeCoRRE::EncodeRect - send an arbitrary size rectangle using CoRRE
 * encoding.
 */

UINT
rdEncodeCoRRE::EncodeRect(BYTE *source, BYTE *dest, const RECT &rect)
{
	// Do the encoding
...
UINT
rdEncodeCoRRE::InternalEncodeRect(BYTE *source, BYTE *dest, const RECT &rect)
{
	int size = 0;
...
void
rdEncodeCoRRE::SetCoRREMax(BYTE width, BYTE height)
{
	m_maxwidth = width;
	m_maxheight = height;
}
...
UINT
rdEncodeCoRRE::EncodeSmallRect(BYTE *source, BYTE *dest, const RECT &rect)
{
	int subrects = -1;
