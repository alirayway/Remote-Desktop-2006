...
// This file implements the rdEncoder-derived rdEncodeZlibHex class.
// This class overrides some rdEncoder functions to produce a
...
rdEncodeZlibHex::rdEncodeZlibHex()
{
...
void
rdEncodeZlibHex::Init()
{
	rdEncoder::Init();
}

UINT
rdEncodeZlibHex::RequiredBuffSize(UINT width, UINT height)
{
	int accumSize;

	// Start with the raw encoding size, which includes the
	// rectangle header size.
	accumSize = rdEncoder::RequiredBuffSize(width, height);
...
UINT
rdEncodeZlibHex::NumCodedRects(RECT &rect)
{
	return 1;
}
...
/*
 * rdEncodeZlibHex::EncodeRect - send a rectangle using hextile encoding.
 */

UINT
rdEncodeZlibHex::EncodeRect(BYTE *source, VSocket *outConn, BYTE *dest, const RECT &rect)
{
...
UINT
rdEncodeZlibHex::zlibCompress(BYTE *from_buf, BYTE *to_buf, UINT length, struct z_stream_s *compressor)
{
 ...
 */																			\
																			...
																			\
UINT																		\
rdEncodeZlibHex::EncodeHextiles##bpp(BYTE *source, BYTE *dest,				\
	...
	{																		\
		...
		{																	\
			...
			{																\
				...
				if ((w*h*(bpp/8)) > rd_ENCODE_ZLIBHEX_MIN_COMP_SIZE)		\
				{															\
			...
			{																\
				...
				if (encodedBytes > (rd_ENCODE_ZLIBHEX_MIN_COMP_SIZE * 2))	\
				{															\
				...
				{															\
					...
					destoffset += encodedBytes;								\
																			...
																			\
		if (destoffset > rd_ENCODE_ZLIBHEX_MIN_DATAXFER)					\
		{																	\
