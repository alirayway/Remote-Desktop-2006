...
// This file implements the rdEncoder-derived rdEncodeHexT class.
...
UINT
rdEncodeHexT::RequiredBuffSize(UINT width, UINT height)
{
	return rdEncoder::RequiredBuffSize(width, height) + (((width/16)+1) * ((height/16)+1));
}

UINT
rdEncodeHexT::NumCodedRects(RECT &rect)
{
	return 1;
}
...
/*
 * rdEncodeHexT::EncodeRect - send a rectangle using hextile encoding.
 */

UINT
rdEncodeHexT::EncodeRect(BYTE *source, BYTE *dest, const RECT &rect)
{
 ...
 */																			\
																			\
UINT																		\
rdEncodeHexT::EncodeHextiles##bpp(BYTE *source, BYTE *dest,				\
