...
#define FLASHER_CLASS_NAME "rdviewer Flasher"
...
Flasher::Flasher(int port)
{
	...
	if (m_hfont == NULL) {
		rdlog.Print(1, _T("FAILED TO SELECT FLASHER FONT!\n"));
	}
	...
	case WM_SOCKEVENT:
		{
			...
			case FD_CLOSE:
				rdlog.Print(2, _T("Flasher connection closed\n"));
				DestroyWindow(hwnd);
				break;
			}
