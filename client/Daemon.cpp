...
//////////////////////////////////////////////////////////////////////
#define DAEMON_CLASS_NAME "rdviewer Daemon"
...
void Daemon::AddTrayIcon() {
	rdlog.Print(4, _T("Adding tray icon\n"));
	SendTrayMsg(NIM_ADD);
}

void Daemon::CheckTrayIcon() {
	rdlog.Print(8, _T("Checking tray icon\n"));
	if (!SendTrayMsg(NIM_MODIFY)) {
		rdlog.Print(4, _T("Tray icon not there - reinstalling\n"));
		AddTrayIcon();
	};
}

void Daemon::RemoveTrayIcon() {
	rdlog.Print(4, _T("Deleting tray icon\n"));
	...
	case WM_SOCKEVENT:
		{
			...
			case FD_CLOSE:
				rdlog.Print(5, _T("Daemon connection closed\n"));
				DestroyWindow(hwnd);
				break;
			}
	...
	case WM_TRAYNOTIFY:
		{
			...
			if (lParam==WM_LBUTTONDBLCLK) {
				...
				if (hSubMenu == NULL) { 
					rdlog.Print(2, _T("No systray submenu\n"));
					return 0;
				}
