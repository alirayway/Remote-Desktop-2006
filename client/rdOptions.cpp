...
// rdOptions.cpp: implementation of the rdOptions class.
...
rdOptions& rdOptions::operator=(rdOptions& s)
{
	for (int i = rfbEncodingRaw; i<= LASTENCODING; i++)
...
void rdOptions::SetFromCommandLine(LPTSTR szCmdLine) {
	...
	for (int j = 0; j < i; j++) {
		...
		// Manual setting of palm vs hpc aspect ratio for dialog boxes.
			...
			if (!ParseDisplay(args[j], phost, 255, &m_port)) {
				ShowUsage(_T("Invalid rd server specified."));
				PostQuitMessage(1);
			} else {
...
void rdOptions::Save(char *fname)
{
	for (int i = rfbEncodingRaw; i<= LASTENCODING; i++) {
		char buf[128];
...
void rdOptions::Load(char *fname)
{
	for (int i = rfbEncodingRaw; i<= LASTENCODING; i++) {
		char buf[128];
		sprintf(buf, "use_encoding_%d", i);
...
// Record the path to the rd viewer and the type
// of the .rd files in the registry
void rdOptions::Register()
{
	char keybuf[_MAX_PATH * 2 + 20];
	HKEY hKey, hKey2;
	if ( RegCreateKey(HKEY_CLASSES_ROOT, ".rd", &hKey)  == ERROR_SUCCESS ) {
		RegSetValue(hKey, NULL, REG_SZ, "rdViewer.Config", 0);
		RegCloseKey(hKey);
	} else {
		rdlog.Print(0, "Failed to register .rd extension\n");
	}

	char filename[_MAX_PATH];
	if (GetModuleFileName(NULL, filename, _MAX_PATH) == 0) {
		rdlog.Print(0, "Error getting rdviewer filename\n");
		return;
	}
	rdlog.Print(2, "Viewer is %s\n", filename);

	if ( RegCreateKey(HKEY_CLASSES_ROOT, "rdViewer.Config", &hKey)  == ERROR_SUCCESS ) {
		RegSetValue(hKey, NULL, REG_SZ, "rdviewer Config File", 0);
	...
	if ( RegCreateKey(HKEY_LOCAL_MACHINE, 
			"Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\rdviewer.exe", 
			&hKey)  == ERROR_SUCCESS ) {
...
void rdOptions::ShowUsage(LPTSTR info) {
    TCHAR msg[2048];
    TCHAR *tmpinf = _T("");
    if (info != NULL) 
        tmpinf = info;
...
#ifdef UNDER_CE
		_T("%s\n\rUsage includes:\n\r")
			_T("rdviewer [/8bit] [/swapmouse] [/shared] [/belldeiconify] \n\r")
			_T(" [/hpc | /palm] [/slow] server[:display] \n\r")
			_T("For full details see documentation."),
#else
		_T("%s\n\rUsage includes:\n\r"
			"  rdviewer [/8bit] [/shared] [/noshared] [/swapmouse] \n\r"
...
#endif
        tmpinf);
    MessageBox(NULL,  msg, _T("rd error"), MB_OK | MB_ICONSTOP | MB_TOPMOST);
}

// The dialog box allows you to change the session-specific parameters
int rdOptions::DoDialog(bool running)
{
	m_running = running;
...
BOOL CALLBACK rdOptions::OptDlgProc(  HWND hwnd,  UINT uMsg,  
									 WPARAM wParam, LPARAM lParam ) {
	// This is a static method, so we don't know which instantiation we're 
	// dealing with. But we can get a pseudo-this from the parameter to 
	// WM_INITDIALOG, which we therafter store with the window and retrieve
	// as follows:
	rdOptions *_this = (rdOptions *) GetWindowLong(hwnd, GWL_USERDATA);
