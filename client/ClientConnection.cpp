...
#define VWR_WND_CLASS_NAME _T("rdviewer")
...
const rfbPixelFormat rd8bitFormat = {8, 8, 0, 1, 7,7,3, 0,3,6,0,0};
const rfbPixelFormat rd16bitFormat = {16, 16, 0, 1, 63, 31, 31, 0,6,11,0,0};
...
ClientConnection::ClientConnection(rdviewerApp *pApp) 
{
	Init(pApp);
}

ClientConnection::ClientConnection(rdviewerApp *pApp, SOCKET sock) 
{
	Init(pApp);
	m_sock = sock;
...
ClientConnection::ClientConnection(rdviewerApp *pApp, LPTSTR host, int port)
{
	Init(pApp);
...
void ClientConnection::Init(rdviewerApp *pApp)
{
	m_hwnd = 0;
	m_desktopName = NULL;
	m_port = -1;
...
void ClientConnection::CreateDisplay() 
{
	...
	if (GetDeviceCaps(m_hBitmapDC, RASTERCAPS) & RC_PALETTE) {
		rdlog.Print(3, _T("Palette-based display - %d entries, %d reserved\n"), 
    ...
    AppendMenu(hsysmenu, MF_SEPARATOR, NULL, NULL);
	AppendMenu(hsysmenu, MF_STRING, IDD_APP_ABOUT,		_T("&About rdviewer..."));
...
void ClientConnection::Connect()
{
	...
	rdlog.Print(0, _T("Connected to %s port %d\n"), m_host, m_port);
...
void ClientConnection::NegotiateProtocolVersion()
{
   ...
   /* if the connection is immediately closed, don't report anything, so
    ...
    try {
		...
		rdlog.Print(0, _T("Error reading protocol version: %s\n"), e.m_info);
    ...
    rdlog.Print(0, _T("RFB server supports protocol version %d.%d\n"),
	    m_majorVersion,m_minorVersion);

    if ((m_majorVersion == 3) && (m_minorVersion < 3)) {
		
        /* if server is 3.2 we can't use the new authentication */
		rdlog.Print(0, _T("Can't use IDEA authentication\n"));
	...
	rdlog.Print(0, _T("Connected to RFB server, using protocol version %d.%d\n"),
...
void ClientConnection::Authenticate()
{
    ...
    case rfbConnFailed:
		...
		rdlog.Print(0, _T("RFB connection failed, reason: %s\n"), m_netbuf);
		throw WarningException(m_netbuf);
        break;
		
    case rfbNoAuth:
		rdlog.Print(0, _T("No authentication needed\n"));
		break;
    ...
    case rfbrdAuth:
		{
			...
			if (strlen((const char *) m_encPasswd)>0) {
				char *pw = rdDecryptPasswd(m_encPasswd);
				strcpy(passwd, pw);
				free(pw);
			} else {
				AuthDialog ad;
...
#endif
				if (strlen(passwd) == 0) {
					rdlog.Print(0, _T("Password had zero length\n"));
					throw AuthException("Empty password");
				}
				if (strlen(passwd) > 8) {
					passwd[8] = '\0';
				}
				rdEncryptPasswd(m_encPasswd, passwd);
			}				
	
			rdEncryptBytes(challenge, passwd);
			...
			switch (authResult) {
			case rfbrdAuthOK:
				rdlog.Print(0, _T("rd authentication succeeded\n"));
				break;
			case rfbrdAuthFailed:
				rdlog.Print(0, _T("rd authentication failed!\n"));
				throw AuthException("rd authentication failed!");
			case rfbrdAuthTooMany:
				rdlog.Print(0, _T("rd authentication failed - too many tries!\n"));
				throw WarningException(
					"rd authentication failed - too many tries!");
			default:
				rdlog.Print(0, _T("Unknown rd authentication result: %d\n"),
					(int)authResult);
				throw ErrorException("Unknown rd authentication result!");
			}
			break;
		}
		
	default:
		rdlog.Print(0, _T("Unknown authentication scheme from RFB server: %d\n"),
			(int)authScheme);
...
void ClientConnection::ReadServerInit()
{
	...
	rdlog.Print(0, _T("Desktop name \"%s\"\n"),m_desktopName);
	rdlog.Print(1, _T("Geometry %d x %d depth %d\n"),
...
void ClientConnection::SizeWindow(bool centered)
{
	...
	rdlog.Print(2, _T("Screen work area is %d x %d\n"), workwidth, workheight);
...
// for rd, but makes scrolling & deiconifying much smoother.
...
void ClientConnection::SetupPixelFormat() {
    ...
    if (m_opts.m_Use8Bit) {		
		...
		rdlog.Print(2, _T("Requesting 8-bit truecolour\n"));  
		m_myFormat = rd8bitFormat;
        ...
        rdlog.Print(2, _T("Requesting 16-bit truecolour\n"));
        m_myFormat = rd16bitFormat;
		...
		if (false) {
			...
			rdlog.Print(2, _T("Memory DC has depth of %d and %s pallete-based.\n"), 
	...
	case WM_KILLFOCUS:
		{
			...
			rdlog.Print(6, _T("Losing focus - cancelling modifiers\n"));
	  ...
	  if (m_waitingOnEmulateTimer)
		   ...
		   || (msg == WM_RBUTTONDOWN && !(keyflags & MK_LBUTTON)))
		{
		  ...
		  if (!m_emulate3ButtonsTimer)
		    {
		      rdlog.Print(0, _T("Failed to create timer for emulating 3 buttons"));
...
inline void ClientConnection::ProcessKeyEvent(int virtkey, DWORD keyData)
{
    ...
    if (GetKeyNameText(  keyData,keyname, 31)) {
        rdlog.Print(4, _T("Process key: %s (keyData %04x): "), keyname, keyData);
    };
#endif
#endif
...
ClientConnection::SendKeyEvent(CARD32 key, bool down)
{
    ...
    rdlog.Print(6, _T("SendKeyEvent: key = x%04x status = %s\n"), key, 
        down ? _T("down") : _T("up"));
}
...
void ClientConnection::SendClientCutText(char *str, int len)
{
	...
	WriteExact(str, len);
	rdlog.Print(6, _T("Sent %d bytes of clipboard\n"), len);
}
#endif
...
void ClientConnection::ShowConnInfo()
{
	...
	_stprintf(
		...
		m_majorVersion, m_minorVersion,
		kbdname);
	MessageBox(NULL, buf, _T("rd connection info"), MB_ICONINFORMATION | MB_OK);
}
...
void* ClientConnection::run_undetached(void* arg) {
	...
	rdlog.Print(9,_T("Update-processing thread started\n"));
	...
	try {
		...
		while (!m_bKillThread) {
			...
			CARD8 msgType;
			{
			  ...
			  if (bytes == 0) {
			    rdlog.Print(0, _T("Connection closed\n") );
			    throw WarningException(_T("Connection closed"));
			  }
			  if (bytes < 0) {
			    rdlog.Print(3, _T("Socket error reading message: %d\n"), WSAGetLastError() );
			...
			case rfbSetColourMapEntries:
		        rdlog.Print(3, _T("rfbSetColourMapEntries read but not supported\n") );
			...
			default:
				if (msgType >= rfbExtensionData) {
	                ...
	                rdlog.Print(3, _T("Unknown message type x%02x\n"), msgType );
        ...
        rdlog.Print(4, _T("Update-processing thread finishing\n") );
...
ClientConnection::SendFramebufferUpdateRequest(int x, int y, int w, int h, bool incremental)
{
	...
	rdlog.Print(10, _T("Request %s update\n"), incremental ? _T("incremental") : _T("full"));
...
void ClientConnection::SendAppropriateFramebufferUpdateRequest()
{
	if (m_pendingFormatChange) {
		rdlog.Print(3, _T("Requesting new pixel format\n") );
...
void ClientConnection::ReadScreenUpdate() {
	...
	for (int i=0; i < sut.nRects; i++) {
		...
		case rfbEncodingZlibHex:
			...
			break;
		default:
			rdlog.Print(0, _T("Unknown encoding %d - not supported!\n"), surh.encoding);
			break;
		}
...
{
	rdlog.Print(5, _T("%s dormant mode\n"), newstate ? _T("Entering") : _T("Leaving"));
	m_dormant = newstate;
	if (!m_dormant)
		SendIncrementalFramebufferUpdateRequest();
}
...
void ClientConnection::ReadServerCutText() {
	...
	rdlog.Print(6, _T("Read remote clipboard change\n"));
...
void ClientConnection::ReadBell() {
	...
	if (! ::PlaySound("rdViewerBell", NULL, 
...
inline void ClientConnection::ReadExact(char *inbuf, int wanted)
{
	...
	int offset = 0;
    rdlog.Print(10, _T("  reading %d bytes\n"), wanted);
	...
	while (wanted > 0) {
		...
		if (bytes == SOCKET_ERROR) {
			...
			rdlog.Print(1, _T("Socket error while reading %d\n"), err);
			m_running = false;
...
inline void ClientConnection::ReadString(char *buf, int length)
{
	...
	buf[length] = '\0';
    rdlog.Print(10, _T("Read a %d-byte string\n"), length);
}
...
inline void ClientConnection::WriteExact(char *buf, int bytes)
{
	...
	rdlog.Print(10, _T("  writing %d bytes\n"), bytes);
    ...
    while (i < bytes) {
		...
		if (j == SOCKET_ERROR || j==0) {
			...
			rdlog.Print(1, _T("Socket error %d: %s\n"), err, lpMsgBuf);
			LocalFree( lpMsgBuf );
			m_running = false;
...
void ClientConnection::CheckBufferSize(int bufsize)
{
	...
	rdlog.Print(4, _T("bufsize expanded to %d\n"), m_netbufsize);
}
...
void ClientConnection::CheckZlibBufferSize(int bufsize)
{
	...
	rdlog.Print(4, _T("zlibbufsize expanded to %d\n"), m_zlibbufsize);
