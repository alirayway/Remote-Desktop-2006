...
typedef struct rdKeymapping_struct {
	UINT wincode;
	UINT Xcode;
} rdKeymapping;
...
static const rdKeymapping keymap[] = {
...
KeyActionSpec KeyMap::PCtoX(UINT virtkey, DWORD keyData) { 
    ...
    rdlog.Print(8,_T(" keyData %04x "), keyData);
    
    if (extended) { 
        rdlog.Print(8, _T(" (extended) "));
        switch (virtkey) {
        case VK_MENU :
    ...
    UINT mapsize = sizeof(keymap) / sizeof(rdKeymapping);
    ...
    if (numkeys != 0) {
        ...
        rdlog.Print(8, _T("keymap gives %u (%x) "), key, key);

    } else {
        // not found in table
        rdlog.Print(8, _T("not in special keymap, "));
        ...
        if (  ((keystate[VK_MENU] & 0x80) != 0) &&
           ...
           if  ( (ret >= 1) && 
               ...
               rdlog.Print(8, _T("Ctrl-Alt pressed: ToAscii (without modifiers) returns %d byte(s): "), ret);
                for (int i = 0; i < ret; i++) {
                   kas.keycodes[numkeys++] = *(buf+i);
                   rdlog.Print(8, _T("%02x (%c) "), *(buf+i) , *(buf+i));
                }
                rdlog.Print(8,_T("\n"));
           } 
        ...
        if (numkeys == 0) {
            ...
            if (ret >= 1) {
                rdlog.Print(8,_T("ToAscii (without ctrl) returns %d byte(s): "), ret);
                for (int i = 0; i < ret; i++) {
                   kas.keycodes[numkeys++] = *(buf+i);
                  rdlog.Print(8, _T("%02x (%c) "), *(buf+i) , *(buf+i));
                }
